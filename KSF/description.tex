...
\subsection{Systematics correction in galaxy surveys}

To make accurate cosmological measurements and detect deviations from \LCDM, we require clean, unbiased datasets.
Observed galaxy catalogs are necessarily imperfect due to systematic effects in observation and instrumentation. 
To characterize the clustering from these surveys, we must model, fit, and remove these effects (e.g. \citealt{Agarwal2014}).
...
\textbf{We are going to simplify these questions of goodness-of-fit to questions about
\emph{anomalies}, or respects in which the data \emph{do not conform}
to the predictions of \LCDM}.
We are suggesting this for two reasons:
The first is that there have been claimed anomalies in the
cosmic microwave background (\CMB) \citep{Huterer2010}, and the conversation around them has been
productive but flawed.
Indeed none of the proposed \CMB\ anomalies is a confident statistical result,
partly because these results are subtle, and
partly because they were discovered in an \foreign{ad hoc} way.
This makes them hard to 
assess in any \foreign{a posteriori} analysis.
The productivity of these results is to be admired, but we propose that the statistical
context needs to be reconsidered.

The second reason for a focus on anomalies is the fact that \textbf{the \LCDM\ model must sure surely be an approximation to a more intricate theory}.
...
\textbf{To first order, in any finite survey volume, these will appear as
\emph{gradients} of statistical properties with respect to three-space
position}.
These kinds of anomalies are the first for which we will search, because they
are natural, and because they are easily enumerated.
...
We need to determine an enumeration of the possibilities.
...
trivial considerations, like which kinds of gradients could be imitated or
...
materials aimed at undergraduates (\UG s) transitioning to graduate school.
...
Contexts for using these materials include \UG-to-\PhD\ bridge programs, courses for advanced \UG\ and early graduate students, and existing research experience programs. 
...
We will use the pedagogical materials to teach intensive workshops on these topics for our target audience of advanced \UG s; the \PI\ has experience designing effective workshops \citep{Huppenkothen2018}.
...
In each summer we will organize one 3-day workshop for senior \UG\ students and beginning
...
These workshops will be advertised through our New York networks, and especially the CUNY system, where the Flatiron Institute has a partnership for \UG s performing astrophysics research.
...
These workshops will be sheduled and planned by the \PI, and executed by the full team, including both the \GRA\ and the \UG\ researcher.

In terms of supervision, the \PI\ will supervise the graduate student and the graduate student will supervise the \UG s.
Of course the \PI\ will also attend to the hiring and supervision of the \UG s.
The \UG\ researchers will do appropriate sub-parts of each of the project themes, like for example executing the estimator on mocks and data sets, gathering or creating mock data, or reading theoretical particle physics papers to find implications for anomalies and observables.
...
on the arXiv which are used for training advanced \UG s and graduate students.
...
The largest drop in continuation of a career in astrophysics for underrepresented minorities is at the \UG-to-\PhD level.
...
The \PI\ will supervise the \GRA's research, and the \GRA\ will mentor the \UG\ researchers.
Research experiences with quality mentorship have been shown to be critical to persistance in \acronym{STEM} for \UG s from underrepresented backgrounds \citep{Estrada2018}.

The interdisciplinary nature of this research means that the educational environment for the \GRA\ and the \UG\ researchers is unique and valuable, especially now.
...
Storey-Fisher contributed substantially to the writing of this proposal; she is a co-author, a proposed beneficiary, and a supervisor (for the \UG\ researchers) of this project.
