...
The \LCDM\ model, then, deserves our respect and our attention. \KSF{i feel like ``respect'' is a bit funny in this context - our recognition?}
By the same token, it deserves our criticism and our scrutiny.
...
with percent-level precision. \KSF{This (previous sentence) is an important sentence but is hard to understand, haven't yet figured out how to break it down}
However, our 1\% precision measurements are not good enough to detect potential deviations from \LCDM\, or to understand subtle cosmological and astrophysical effects.
...
this signature will appear at sub-percent level in the
expansion history and the growth of structure. \KSF{How do we know this observationally when we don't have the observations?} \KSF{Galaxy formation is another example that will be more relevant in the rest of the proposal}

\KSF{I think it's odd that the reader doesn't know what we're proposing even broadly until the 3rd paragraph. Consider adding general ``thesis'' sentence at end of 1st / beginning of 2nd? Or is this standard for long proposals?}
%For us, these are just examples of things we might be doing in the coming decade.
Here we propose to design, build, and exercise tools that will be
valuable for answering these types of cosmological questions, even as the questions themselves evolve and expand.
...
\textbf{departures from predictions}. \KSF{Do we need the qualifier ``in our view''?}
...
\LCDM\ model. \KSF{I really like this paragraph!}

\KSF{Volume sort of implies large-scale structure or related (supernovae, lensing) probes, as opposed to other cosmological probes (CMB, GWs) - should mention the types of data we're focusing on in the previous paragraph? We say LSS at end but it seems like we're implying cosmology is LSS}

Doing better than has ever been done before will require work along
multiple axes. \KSF{Don't love the general-ness of ``doing'' but hard to think of alternative. ``Making measurements that give this next level of insight will require...''}
The first axis is \emph{volume}: We need to observe more spatial volume. \KSF{data volume and spatial volume?}
This is happening now, with projects like the eBOSS survey within the Sloan Digital Sky Survey (SDSS) surveying the largest volume to date \citep{Dawson2015}. 
Upcoming wide-field surveys include DESI \citep{Aghamousa2016}, which will begin taking data in 2019, followed by NASA's Euclid mission \citep{Amiaux2012} and LSST \citep{Ivezic2018} in 2021, and WFIRST in the mid-2020s \citep{Akeson2019}. \KSF{Spell out acronyms?}
These will observe 10-20 times more galaxies than current surveys,
...
discoverable anomalies. \KSF{Which two issues? Unclear}
...
we need to have a very solid statistical plan. \KSF{These three axes: Big Bad Wolf (Big data, bad data, trickster data) :)}
...
methods for window function estimation. \KSF{for systematics correction? is window function the main operator here?}
...
new physics.
\KSF{Feel like need a powerful sentence to send off into the themes, but hard without repeating first sentence of this paragraph}
...
that Storey-Fisher WAS SUPPORTED PARTIALLY BY WHAT PREVIOUSLY? \KSF{Um I've been supported by the NSF during REUs, and NSF partially funded the Kavli Summer Program in Astrophysics I did, but not sure if those count}
...
\textit{Isn't it ridiculous to look for anomalies in the large-scale structure?} \KSF{What kinds of anomalies would you be most likely to detect?}
...
\textit{Don't you still have to make choices about your basis functions for your estimator, so you're not totally eliminating bining?}

\textit{Are you saying that the canonical two-point correlation function estimator is wrong?} \KSF{Well kind of, but that's not what this estimator is solving}

\textit{Aren't there already methods for dealing with nonlinear systematics?} \KSF{Yes, but they're still inadequate, and there aren't methods for dealing with combinations of systematics (I don't think...)}

\textit{What is the connection between systematics correction and anomaly detection?}

\textit{Isn't a brute-force, comprehensive search for \LSS anomalies impossible due to computational cost?}

\textit{Will pedagogical publications and workshops really have a broad impact?}
