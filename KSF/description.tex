...
% - decide where to bold, if at all
...
\KSF{Move this sentence to systematics section?}
...
\KSF{The two above paragraphs could be condensed into one}
...
physics, such as star formation and feedback.
\KSF{Added bullock citation here but not sure if this is what you were thinking - its ``Small-Scale Challenges to the LCDM Paradigm''}
...
\KSF{do we need ``for the intial conditions''?}
\KSF{Reading further, i may have misunderstood - do you want to scope only to two-point statistics? if so we should be explicit about that}
...
\item
Search for gradients in all of the statistics in the mock
...
The estimator is so flexible that the basis functions can be chosen to include not only separation, 
...
survey.  The basis functions in $s$ can be
...
\KSF{I thought the expl was too confusing for someone who hasn't been thinking about these things, but lmk if i've changed the meaning too much.}
Given that we need the systematics modeling for the injection tests,
and our unbinned estimator to measure the gradients,
...
The investigators are well-versed in inquiry-based teaching practices, which are shown to increase engagement and understanding, with a focus on equity and inclusivity \citep{Ball2010}.
...
With feedback from these workshops, we will iterate on the materials, and finally distribute and advertise them online. 
...
The grant supported or partially supported
...
One of the most limiting aspects of \cf analyses is the number of mock catalogs required for error estimation, which are built from expensive cosmological simulations. This depends on the number of bins or basis functions; our estimator reduces this without sacrificing accuracy, significantly reducing cost.
...
The materials we produce are designed to impact students at a critical point in their careers. Access to these materials, alongside well-taught workshops, have the potential to give students with less experience and resources the necessary grounding to do high-quality research in data-driven astrophysics. Further, some of the \PI's pedagogical contributions (especially on cosmological distances and fitting models) have had very wide impact.
