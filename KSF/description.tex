...
\xi(T_l, T_{l'}) &\equiv& a\T \cdot f(T_l, T_{l'}) \quad ,
\end{eqnarray}
...
\KSF{all the parenthetical make this difficult to read, don't think you need all}
...
\KSF{What does this mean? Who is we and in what contexts are we ``good'' at it?}
...
\KSF{Combine these two paragraphs above and below?}
...
\KSF{above sentence is confusing. suggest: The question of whether data fits a model is a statistics question as old as the question of parameter estimation, and has even deeper roots in cosmology.}
...
\KSF{fixed-->reconsidered? revisited?}
...
physics (\HOGG{CITE}).
\KSF{What does interference mean here?}
...
\KSF{mocked-->denerate with? imitated by? confused with?}
...
statistical homogeneity---there is quite a workflow.
\KSF{You're making it sound less concrete than it is (though still not very!) suggest ``there is significant work to be done.''}
...
\KSF{Can we just choose this for now to make this more concrete}
\item
...
\KSF{Null if the mocks were noiseless, but we will detect weird things due to random noise/fluctuations, no?}
\KSF{I don't know if we need to say test code? Maybe ``will act as a check of our methodology''?}
\item
...
\item
Operate on the real data, visualize, write up and publish results of the
analysis with the pre-registered code.
\KSF{say eBOSS data here?}
\item
...
\KSF{kill or rephrase ``orthogonal probably''}

The execution of this theme depends on the products of the
previous two themes.
\KSF{why is this sentence hanging out here by itself}
...
\KSF{Throwing things around here, feel free to cut...}
